...
export default function CometOverlay({ expanded, onClose }) {
  return (
    <AnimatePresence>
      {expanded && (
        <motion.div
...
50"
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-black mb-4">Comet ID: {expanded?.id}</p>
            <p className="mb-4 text-black">{expanded?.description}</p>
            <button
              onClick={onClose}
              className="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 
transition-colors"
              aria-label="Close Comet overlay"
              title="Close"
            >
              Close
            </button>
