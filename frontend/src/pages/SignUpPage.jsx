...
.sup-grid{
  display:grid; gap:16px; grid-template-columns: 140px 1fr;
}

/* Side nav hidden on mobile; mobile pills shown instead */
.side-nav{ position:sticky; top:16px; align-self:flex-start; width:140px; }
.mobile-steps{
  display:none;
}
...
.pills{
  ...
  padding:8px; background:#0b0b0b; border:1px solid #222; border-radius:10px;
  position:sticky; top:8px; z-index:5; box-shadow:0 2px 12px rgba(0,0,0,.35);
}
.pill-btn{
  white-space:nowrap; padding:10px 14px; border-radius:999px; border:1px solid #2d2d32;
  background:#232326; color:var(--text); font-weight:800; cursor:pointer;
}
...
/* Small helpers */
.section { padding:0 0 18px 0; margin-bottom:28px; }
.section h2 { font-size:1.35rem; font-weight:800; margin:0 0 14px 0; color:var(--gold); letter-spacing:.5px; }
.err { color:#f87171; font-size:.98rem; margin-top:4px; }
.preview { object-fit:cover; border-radius:10px; border:1px solid #2d2d32; }
`;
...
export default function SignUpPage() {
  ...
  /* ----- optional username availability ----- */
  ...
  async function submit(e) {
    ...
    try {
      ...
      // ensure cookie/session recognized
      try { await fetch(ME_URL, { credentials: "include" }); } catch {}
  ...
  const scrollTo = (key) => refs[key].current?.scrollIntoView({ block: "start" });
  ...
  return (
    <>
      ...
      <div className="mobile-steps">
        <div className="pills">
          {SECTIONS.map((s) => (
            <button key={s.key} type="button" className="pill-btn" onClick={() => scrollTo(s.key)}>
              {s.label}
            </button>
          ))}
        </div>
        ...
        <div className="sup-grid">
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {SECTIONS.map((s) => (
                <li key={s.key} style={{ marginBottom: 10 }}>
                  <button type="button" onClick={() => scrollTo(s.key)} style={navBtn}>
                    {s.label}
                  </button>
                </li>
              ))}
          ...
          <div style={{ flex: 1, maxWidth: 900, margin: "0 auto" }}>
            ...
            <form onSubmit={submit} autoComplete="off" style={{ color: "#ffe259" }}>
              {/* Step 1 */}
              <section ref={refs.basic} className="section">
                ...
                {errors.birthday && <div className="err">{errors.birthday}</div>}
              ...
              {/* Step 2 */}
              <section ref={refs.security} className="section">
              ...
              {/* Step 3 */}
              <section ref={refs.profile} className="section">
                ...
                {errors.profilePic && <div className="err">{errors.profilePic}</div>}
              ...
              {/* Step 4 */}
              <section ref={refs.referral} className="section">
